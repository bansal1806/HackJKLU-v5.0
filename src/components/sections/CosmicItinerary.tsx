...
function getCoordinates(day: 'day1' | 'day2' | 'day3', index: number, total: number, viewportWidth: number, viewportHeight: number): { x: number; y: number } {
...
export function CosmicItinerary() {
  ...
  const starPositions = currentEvents.map((_, i) => getCoordinates(activeDay, i, currentEvents.length, dimensions.width, dimensions.height));
  ...
  useEffect(() => {
    ...
    const pathData = starPositions.map((pos, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1">
                  <div className="text-xs font-[Cinzel] text-white/80 mb-1">{event.time}</div>
                  <div className="text-lg font-[Cinzel] text-white font-semibold">{event.title}</div>
                </div>
              </div>
            );
          })}
